...
define("FGC_CMD_PATH","~/fantasygold_new/src/./fantasygold-cli"); //fantasygold-cli path
define("MAIN_FGC_ADDRESS","FDyNS7WwaNQELNRKZFoUVkNRttyM61u356"); //change main FGC wallet address

define("TOKEN_CONTRACT_ADDRESS","b3d16bf4ccf764fd28325df28310f2c77aef2f2b"); //FGC contract address
define("TOKEN_DECIMALS","8"); //FGC decimals
...
define("DEFAULT_FEE_AMOUNT",0.01); //default fee to send to deposit address before moving tokens out if the address does not have enough FGC for gas
define("MIN_BALANCE_BEFORE_REFUND",0.002); //min FGC balance a deposit address has to have, if not DEFAULT_FEE_AMOUNT will be sent to it
...
function printHelp(){
    echo "Usage:".PHP_EOL;
    echo "getNewDepositAddress: php fgc20.php getNewDepositAddress".PHP_EOL;
    echo "sendTokenToAddress address amount: php fgc20.php sendTokenToAddress FVpCzEFBznr1XseP9vr7VFMfsyNNrzGBsh 112.12345678".PHP_EOL;
    echo "getTokenBalance address: php fgc20.php getTokenBalance FVpCzEFBznr1XseP9vr7VFMfsyNNrzGBsh".PHP_EOL;
    echo "getAddressDeposits address startingblock: php fgc20.php getAddressDeposits FVpCzEFBznr1XseP9vr7VFMfsyNNrzGBsh 500".PHP_EOL;
    echo "getDepositConfirmations txid: php fgc20.php getDepositConfirmations a5206544436b5ba31d8261f37065f43cdbceb3641a08a43bf9b45e4ac184f6f5".PHP_EOL;
    echo "moveDeposits: php fgc20.php moveDeposits".PHP_EOL;
}
// examples
//echo getNewDepositAddress();
//echo sendTokenToAddress("FVpCzEFBznr1XseP9vr7VFMfsyNNrzGBsh","112.12345678");
//echo getTokenBalance("FdyNS7WwaNQELNRKZFoUVkNRttyM61u356");
//print_r(getAddressDeposits("FVpCzEFBznr1XseP9vr7VFMfsyNNrzGBsh",0));
//echo getDepositConfirmations('73f7a2dd0bc8dfc4bab9309d281edd2f9daca75883752d19fca1ac11031aac84');
//echo getAddressBalance('Fcr9qdFWEcvhy82j58LyoapkZpf5vdnzx9').PHP_EOL;
...
function sendTokenToAddress($userAddress,$amount){
    ...
    $result=json_decode(trim(sendCmd('sendtocontract '.TOKEN_CONTRACT_ADDRESS.' '.'a9059cbb'.to32bytesArg(addressToHash160($userAddress)).to32bytesArg(addDecimals($amount)).' 0 '.DEFAULT_GAS_LIMIT.' '.DEFAULT_GAS_PRICE.' '.MAIN_FGC_ADDRESS)),true)["txid"];
    return $result;
}
...
function moveDeposits(){
    ...
    foreach($addresses as $address){
        ...
        if(validateAddress($address) && $amount>=MIN_DEPOSIT_MOVE_AMOUNT){
            if(getAddressBalance($address)<MIN_BALANCE_BEFORE_REFUND)sendCmd('sendtoaddress '.$address.' '.DEFAULT_FEE_AMOUNT); // send fantasygold fee to the depositAddress so we can move the token out
            $results[]=array('from'=>$address,'to'=>MAIN_FGC_ADDRESS,'Tokens'=>$amount,'txid'=>json_decode(trim(sendCmd('sendtocontract '.TOKEN_CONTRACT_ADDRESS.' '.'a9059cbb'.to32bytesArg(addressToHash160(MAIN_FGC_ADDRESS)).to32bytesArg(addDecimals($amount)).' 0 '.DEFAULT_GAS_LIMIT.' '.DEFAULT_GAS_PRICE.' '.$address)),true)["txid"]);           
            $count++;
